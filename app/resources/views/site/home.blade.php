...
<head>
    ...
    <meta name="robots" content="{{ ($page->seo['indexable'] ?? true) ? 'index,follow' : 'noindex,nofollow' }}">
    <link rel="canonical" href="{{ $locale === 'ar' ? route('home.ar') : route('home.en') }}">
    <link rel="alternate" hreflang="ar" href="{{ route('home.ar') }}">
    <link rel="alternate" hreflang="en" href="{{ route('home.en') }}">
    <link rel="alternate" hreflang="x-default" href="{{ route('home.ar') }}">
    <meta property="og:type" content="website"><meta property="og:locale" content="{{ $locale === 'ar' ? 'ar_SA' : 'en_US' }}">
    <meta property="og:title" content="{{ $page->seo('og_title') }}"><meta property="og:description" content="{{ $page->seo('og_description') }}">
    <meta property="og:url" content="{{ $locale === 'ar' ? route('home.ar') : route('home.en') }}"><meta property="og:image" content="{{ $page->media($page->seo['og_image'] ?? $page->get('brand.logo')) }}">
    <meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="{{ $page->seo('og_title') }}"><meta name="twitter:description" content="{{ $page->seo('og_description') }}">
    <style>:root{@foreach($page->theme as $key=>$value)--{{ str_replace('_','-',$key) }}:{{ $value }};@endforeach}</style>
    @vite(['resources/css/site.css', 'resources/js/site.js'])
    @php
        $l = fn($value) => is_array($value) ? ($value[$locale] ?? $value['ar'] ?? reset($value)) : $value;
        $schema = ['@context'=>'https://schema.org','@type'=>['Person','ProfessionalService'],'name'=>$page->trans('brand.name'),'jobTitle'=>$page->seo['schema']['job_title'][$locale] ?? '', 'areaServed'=>$page->seo['schema']['service_area'] ?? 'Saudi Arabia','url'=>$locale === 'ar' ? route('home.ar') : route('home.en')];
    @endphp
    <script type="application/ld+json">{!! json_encode($schema, JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES) !!}</script>
</head>
<body>
@if(!empty($isPreview))<div class="preview-banner">{{ $locale === 'ar' ? 'أنت تشاهد نسخة المعاينة' : 'You are viewing a preview' }}</div>@endif
<div class="loader" id="loader" aria-hidden="true"><div class="loader-mark"></div></div>
<div class="progress-bar" id="progress"></div>
<div class="site-shell">
    <nav class="nav">
        <a class="brand" href="#top" aria-label="{{ $page->trans('brand.name') }}"><span class="brand-mark"><img src="{{ $page->media($page->get('brand.logo')) }}" alt="{{ $page->trans('brand.name') }}"></span><span>{{ $page->trans('brand.name') }}</span></a>
        <button class="nav-toggle" type="button" aria-label="Menu" aria-expanded="false"><i class="fa-solid fa-bars"></i></button>
        <div class="nav-links" aria-label="Navigation">
            @foreach($page->items('navigation') as $item)<a href="#{{ $item['anchor'] }}">{{ $l($item['label']) }}</a>@endforeach
            <a href="{{ $locale === 'ar' ? route('home.en') : route('home.ar') }}" aria-label="Language">{{ $locale === 'ar' ? 'EN' : 'العربية' }}</a>
        </div>
    </nav>

    <header class="hero" id="top">
        <canvas id="hero-web" aria-hidden="true"></canvas>
        <div class="hero-kinetic-words" aria-hidden="true">
            @foreach($page->get('hero.words', []) as $index => $word)
                @php($positions=[['11%','7%'],['18%','28%'],['13%','68%'],['34%','5%'],['45%','18%'],['33%','43%'],['38%','76%'],['62%','8%'],['69%','31%'],['58%','60%'],['77%','73%'],['82%','15%'],['24%','86%'],['86%','48%']])
                <span class="kinetic-word" data-depth="{{ 30 + (($index * 7) % 45) }}" data-spin="{{ $index % 2 ? -5 : 5 }}" style="--top:{{ $positions[$index][0] ?? '50%' }};--right:{{ $positions[$index][1] ?? '50%' }};--size:{{ .88 + (($index % 4) * .09) }}rem;--delay:-{{ $index * .7 }}s;--duration:{{ 12 + ($index % 5) }}s;--drift-x:{{ $index % 2 ? 40 : -44 }}px;--drift-y:{{ $index % 3 ? 28 : -28 }}px;--drop-y:20px;--tilt:{{ $index % 2 ? 7 : -7 }}deg;--tilt-mid:{{ $index % 2 ? -5 : 5 }}deg"><span class="kinetic-word-label">{{ $word }}</span></span>
            @endforeach
        </div>
        <div class="hero-grid">
